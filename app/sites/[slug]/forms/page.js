...
const Pages = () => {
    ...
    const fetchAllPages = async () => {
        try {
            const response = await axios.get(`/api/getallforms?site_id=${slug}`, {
                headers: {
    ...
    const handleCreateForm = () => {
        router.push(`/sites/${slug}/forms/Id`);
    };
    ...
    return (
        <div>
            ...
            <div className="flex justify-end mt-4 mr-4">
                <button onClick={handleCreateForm} className="bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Create Form
                </button>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-10 mx-2 ml-15">
                    {pages.map(page => (
                        <div
                            ...
                            onClick={() => handleFormClick(page.id)}
                        >
