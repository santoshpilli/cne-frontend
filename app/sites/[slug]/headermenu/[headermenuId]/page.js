...
const PageEditor = () => {
    const { slug, headermenuId } = useParams();
    ...
    useEffect(() => {
        if (headermenuId !== "Id") {
            fetchPageDetails(headermenuId);
        }
    }, [slug, headermenuId]);
  

    const fetchPageDetails = async (headermenuId) => {
     
        try {
            const response = await axios.get(`/api/getheadermenudetails?headermenuId=${headermenuId}`, {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('/api/createheadermenu', data, {
                headers: {
                    'Content-Type': 'application/json',
                },
            });
    ...
    const updatePage = async () => {
        ...
        try {
            const response = await axios.post(`/api/updateheadermenu?headermenuId=${id}`, data, {
                headers: {
                    'Content-Type': 'application/json',
                },
            });
                ...
                <div style={{ width: '100%' }}>
                    ...
                    <div className="flex space-x-4 p-4">
                        <div>
                            ...
                            <label className="block text-lg font-medium text-gray-700 mt-2">HeaderMenu</label>
                            <Input
                                value={title}
